...
if (! function_exists('opportunityManagedBy')) {
    function opportunityManagedBy(User $operator): Opportunity
    {
...
it('AC-007: PATCH the work panel with a different source_id -> 422, value unchanged', function () {
    ...
    $opportunity = opportunityManagedBy($actor);
...
it('AC-008: PATCH the work panel with the SAME source_id -> 200, other fields saved (change-based)', function () {
    ...
    expect($opportunity->fresh()->source_id)->toBe($opportunity->source_id)
        ->and($opportunity->fresh()->next_callback_at?->toDateString())->toBe('2026-09-01');
});
...
it('AC-011: an actor with updateSource can change the Fonte from the work panel', function () {
    ...
    $this->patchJson("/api/request-management/{$opportunity->id}", ['source_id' => $newSource->id])
        ->assertOk();

    expect($opportunity->fresh()->source_id)->toBe($newSource->id);
});
...
it('AC-012: a request reassigned to a new operator without updateSource still 422s on the Fonte', function () {
    ...
    $opportunity = opportunityManagedBy($previousOperator);
    ...
    expect($opportunity->fresh()->source_id)->not->toBe($otherSource->id);
});
