...
| like approve/block already do, so the admin-as-assignee deroga (0116 D-2)
| survives the super-admin Gate::before. The UI flag, the grid row-action and
| the bulk-delete per-row authorization read the SAME rule.
*/
...
it('AC-001: a super-admin who is an assignee (not creator/requester) gets 403 on DELETE, task kept', function () {
...
it('AC-004: a super-admin assignee of a task WITH sub-tasks gets 403, not 409', function () {
    $actor = superAdminTaskActor();
    $task = Task::factory()->create();
    $task->assignees()->attach($actor->id);
...
it('AC-005: bulk-delete by a super-admin assignee reports the task as forbidden and keeps it', function () {
    $actor = superAdminTaskActor();
...
it('AC-007: a super-admin assignee has no delete flag nor delete row-action; a non-assignee super-admin has both', function () {
    $assignedAdmin = superAdminTaskActor();
    $otherAdmin = superAdminTaskActor();
    $task = Task::factory()->create();
    $task->assignees()->attach($assignedAdmin->id);

    Sanctum::actingAs($assignedAdmin);
    $this->getJson("/api/tasks/{$task->id}")->assertOk()->assertJsonPath('permissions.actions.delete', false);
