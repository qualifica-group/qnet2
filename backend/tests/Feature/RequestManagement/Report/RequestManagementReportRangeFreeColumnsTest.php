...
it('AC-001: the catalogue ends with the three range-free columns', function (): void {
    expect(array_slice((array) config('request-management-report.indicator_columns'), -3))
        ->toBe(['unhandled_callbacks', 'unhandled_new_contacts', 'current_potentials']);
});
...
it('AC-006: range-bound labels carry the period suffix, range-free ones keep the original names', function (): void {
    app()->setLocale('it');

    expect(__('request-management-report.headers.richiami'))->toBe('N. Richiami non gestiti (nel periodo)')
        ->and(__('request-management-report.headers.telefonate'))->toBe('N. Telefonate Effettuate (nel periodo)')
        ->and(__('request-management-report.headers.aule_gestione'))->toBe('Aule in gestione')
