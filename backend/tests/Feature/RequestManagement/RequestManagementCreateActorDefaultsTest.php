...
/**
 * An actor whose employment profile holds a freshly created Sede as its
 * PHYSICAL membership on the `employment_profile_operational_site` pivot
 * (spec 0103, AC-032).
 *
 * Built off EmploymentProfileFactory::raw() with the still-present
 * `operational_site_id` key stripped (that dead column no longer exists on
 * the table; the factory's own cleanup is microtask M11, out of this lane's
 * scope) instead of the usual factory()->create(), which would otherwise
 * force-fill that key straight into the insert.
 */
function anActorAtOwnSite(string ...$abilities): array
{
    ...
    $employment = EmploymentProfile::query()->create(
        Arr::except(EmploymentProfile::factory()->raw(['user_id' => $actor->id]), ['operational_site_id'])
    );
