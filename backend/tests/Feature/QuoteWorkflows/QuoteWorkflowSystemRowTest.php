...
it('update: 422 when a system row\'s group is changed via statuses sync (AC-006)', function () {
    ...
    $this->assertDatabaseHas('quote_workflow_statuses', ['id' => $openId, 'group' => 'open']);
});
...
it('update: omitting a system row from the statuses sync does NOT delete it (AC-006)', function () {
    ...
    $this->assertDatabaseHas('quote_workflow_statuses', ['id' => $openId, 'system_key' => 'open']);
    $this->assertDatabaseHas('quote_workflow_statuses', ['id' => $closedId, 'system_key' => 'closed_won']);
});
...
it('update: a custom row omitted from the statuses sync is deleted (AC-007)', function () {
    ...
    $this->assertDatabaseMissing('quote_workflow_statuses', ['name' => 'Drop']);
    $this->assertDatabaseHas('quote_workflow_statuses', ['id' => $keep['id']]);
});
