...
it('200 with the catalogue for users and roles, keys matching each resolver\'s fields()', function () {
    ...
    expect($resources->keys()->all())->toEqualCanonicalizing([
        ...
        // only on create, D-1).
        'units-of-measure',
        // spec 0099 `product-typologies` (ProductTypologiesAuthorization: the
        // "Tipologie Prodotto" lookup — name mandatory, `code` writable only
        // on create, D-2).
        'product-typologies',
...
it('spec 0008/0015/0111/0129: users.fields contains exactly the 4 existing + 13 personal_data.* + 14 employment.* keys, with the contracted type/group', function () {
    $actor = actorWithRoleAbilities(['create']);
    Sanctum::actingAs($actor);
