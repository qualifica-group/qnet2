...
function workflowWithoutValidatedRow(string $name): array
{
    ...
    return test()->postJson('/api/opportunity-workflows', [
        'name' => $name,
...
it('update: marking a custom row makes it the validated system row, ordered before closed_won', function () {
    Sanctum::actingAs(opportunityWorkflowUserWith(['create', 'update']));
...
it('default statuses: the global set moves the mark onto another row', function () {
    Sanctum::actingAs(opportunityWorkflowUserWith(['viewAny', 'view', 'update']));

    $rows = collect($this->getJson('/api/opportunity-workflows/default-statuses')->assertOk()->json('data'));
    ...
    $data = collect($this->putJson('/api/opportunity-workflows/default-statuses', ['statuses' => $payload])->assertOk()->json('data'));
