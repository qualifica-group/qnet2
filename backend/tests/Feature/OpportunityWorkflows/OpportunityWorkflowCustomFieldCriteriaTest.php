...
// OpportunityWorkflowCustomFieldOnCreateTest).
...
if (! function_exists('companyRelationCustomField')) {
     ...
     * same-named helper in OpportunityWorkflowCustomFieldResolverTest.php:
...
// ---------------------------------------------------------------------------
// AC-027/AC-028 — GET /api/opportunity-workflows/criterion-fields
// ---------------------------------------------------------------------------
...
it('create: 201 with a custom relation criterion, persisted with field=custom.<key> (AC-030)', function () {
    ...
    $this->assertDatabaseHas('opportunity_workflow_criteria', [
...
it('update: PATCH re-syncs a custom relation criterion (AC-030)', function () {
    ...
    $this->patchJson("/api/opportunity-workflows/{$created}", [
    ...
    $this->assertDatabaseMissing('opportunity_workflow_criteria', ['field' => 'source_id']);
});
...
it('show/table stay 200 after the referenced custom field definition is disabled (AC-034)', function () {
    $definition = companyRelationCustomField();
    $company = Company::factory()->create();
    $actor = opportunityWorkflowUserWith(['view', 'viewAny']);
    Sanctum::actingAs($actor);
    ...
    $this->getJson("/api/opportunity-workflows/{$workflow->id}")->assertOk()
    ...
    $this->postJson('/api/tables/opportunity-workflows/rows', ['startRow' => 0, 'endRow' => 25])
        ->assertOk();
});
...
it('show/table stay 200 after the referenced custom field definition is deleted (AC-034)', function () {
    ...
    $this->getJson("/api/opportunity-workflows/{$workflow->id}")->assertOk();
    $this->postJson('/api/tables/opportunity-workflows/rows', ['startRow' => 0, 'endRow' => 25])->assertOk();
});
...
// 'opportunityWorkflows.criterionFields.' prefix means "translate me".
// ---------------------------------------------------------------------------
...
it('table rows: criteria_fields emits the i18n key for a native criterion and the literal label for a custom one (AC-037)', function () {
    ...
    expect($row['criteria_fields'])->toEqualCanonicalizing([
        'opportunityWorkflows.criterionFields.source_id',
        'Azienda preferita',
    ]);
});
