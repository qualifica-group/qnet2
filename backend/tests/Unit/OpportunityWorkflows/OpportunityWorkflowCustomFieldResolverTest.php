...
// OpportunityWorkflowResolverTest's own helpers (separate file: that one
// covers the base spec's native-field ACs, this one the amendment).
...
if (! function_exists('companyRelationCustomField')) {
    ...
    // same-named helper in OpportunityWorkflowCustomFieldCriteriaTest.php:
...
it('matches a cardinality-one custom relation criterion on the exact value, not on a different one or null', function () {
    ...
    expect(workflowResolver()->resolve($matchingOpportunity)?->id)->toBe($matchingWorkflow->id);
    ...
    expect(workflowResolver()->resolve($differentValueOpportunity))->toBeNull();

    $noValueOpportunity = Opportunity::factory()->create();
    expect(workflowResolver()->resolve($noValueOpportunity))->toBeNull();
});
...
it('matches a cardinality-many custom relation criterion when the value is contained in the array (D7)', function () {
    ...
    expect(workflowResolver()->resolve($opportunity)?->id)->toBe($matchingWorkflow->id);
