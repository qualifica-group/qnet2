 ...
 * the EFFECTIVE (own + inherited) attribute rows for one product category, or
 * the UNION (deduped by `code`) across every category — each memoized per
 * instance (one `RequestManagementScopedTableDefinition` per request, see
 * TableRegistry::resolve()) so repeated calls within one request never re-walk
 * the hierarchy.
 *
 * The context is `AttributeContext::Quote` (user directive 2026-08-31, NOT
 * the `Opportunity` context spec 0064 originally read): spec 0084 D-1 moved
 * "Informazioni aggiuntive" onto the Offerta, and the row of this grid IS an
 * Offerta since spec 0086 — so the grid's flexible columns must resolve
 * exactly the set the work panel and the Offerte form resolve
 * (App\RequestManagement\RequestAttributeResolver), never a second,
 * divergent one.
 *
...
final class AttributeScopeResolver
{
     ...
     * Quote attributes (spec 0064, D-4: the preferences/filters
    ...
    private function resolveForCategory(int $categoryId): Collection
    {
        ...
        return self::sortBySortOrderThenCode($this->hierarchy->effectiveAttributes($category, AttributeContext::Quote));
    }
