...
/**
 * ResourceAuthorization for the `task-types` resource (spec 0101, D-4).
 *
 ...
 */
class TaskTypesAuthorization extends AbstractResourceAuthorization
{
    ...
    {
        return [
            'delete' => $model !== null && $actor->can('task-types.delete'),
            'export' => $actor->can('task-types.export'),
            'import' => $actor->can('task-types.import'),
            // Gates the ActivityLogSection in the detail (spec 0034); the
            // record-level `task-types.view` boundary is enforced separately by
            // GET /api/activity-log/task-types/{id}.
            'view_activity' => $model !== null && $actor->can('task-types.viewActivity'),
        ];
    }
}
