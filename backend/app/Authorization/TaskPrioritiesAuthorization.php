...
/**
 * ResourceAuthorization for the `task-priorities` resource (spec 0101, D-4).
 *
 ...
 */
class TaskPrioritiesAuthorization extends AbstractResourceAuthorization
{
    ...
    {
        return [
            'delete' => $model !== null && $actor->can('task-priorities.delete'),
            'export' => $actor->can('task-priorities.export'),
            'import' => $actor->can('task-priorities.import'),
            // Gates the ActivityLogSection in the detail (spec 0034); the
            // record-level `task-priorities.view` boundary is enforced separately by
            // GET /api/activity-log/task-priorities/{id}.
            'view_activity' => $model !== null && $actor->can('task-priorities.viewActivity'),
        ];
    }
}
