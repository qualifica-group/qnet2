...
/**
 * ResourceAuthorization for the `task-importances` resource (spec 0101, D-4).
 *
 ...
 */
class TaskImportancesAuthorization extends AbstractResourceAuthorization
{
    ...
    {
        return [
            'delete' => $model !== null && $actor->can('task-importances.delete'),
            'export' => $actor->can('task-importances.export'),
            'import' => $actor->can('task-importances.import'),
            // Gates the ActivityLogSection in the detail (spec 0034); the
            // record-level `task-importances.view` boundary is enforced separately by
            // GET /api/activity-log/task-importances/{id}.
            'view_activity' => $model !== null && $actor->can('task-importances.viewActivity'),
        ];
    }
}
