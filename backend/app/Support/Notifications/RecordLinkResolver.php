...
final class RecordLinkResolver
{
    public static function pathFor(User $notifiable, AssignmentTargetEnum $target, int $recordId): ?string
    {
        return match ($target) {
            AssignmentTargetEnum::Registry => $notifiable->can('registries.view')
                ? "/registries/{$recordId}"
                : null,
            AssignmentTargetEnum::Opportunity => self::opportunityPath($notifiable, $recordId),
        };
    }
     ...
     */
    private static function opportunityPath(User $notifiable, int $recordId): ?string
    {
        ...
        if ($notifiable->can('request-management.view')) {
            return "/request-management/{$recordId}";
        }
