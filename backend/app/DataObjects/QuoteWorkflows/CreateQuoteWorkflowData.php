...
/**
 * Validated payload for creating a quote workflow (POST
 * /api/quote-workflows, spec 0047, moved onto the Offerta by spec 0083 D-6).
 * Declared DTO (no "magic flying array") so the StoreQuoteWorkflowRequest ->
 * QuoteWorkflowService contract is explicit.
 *
 * `criteria` is REQUIRED, min:1 (AC-008): a list of {field, value_id} pairs,
 * one per allow-listed field (App\Support\QuoteWorkflows\
 * QuoteCriterionFieldRegistry). `statuses` is OPTIONAL. The mandatory system
 * rows (open/closed_won/closed_lost, AC-004) are always created by the
...
final readonly class CreateQuoteWorkflowData
{
    ...
    /**
     * Build from the validated StoreQuoteWorkflowRequest payload.
     *
