...
class ExportController extends BaseApiController
{
    ...
    public function store(CreateExportRequest $request, string $domain): JsonResponse
    {
        try {
            ...
            // `opportunityId` (spec 0067, D-5)/`quoteId` (spec 0095, D-8)/
            // `workOrderId` (spec 0133) are
            ...
            $state = $request->safe()->only(['columns', 'sortModel', 'filterModel', 'search', 'opportunityId', 'quoteId', 'workOrderId']);
