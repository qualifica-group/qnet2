...
use App\Http\Requests\Tasks\UnblockTaskRequest;
...
/**
 * POST /api/tasks/{task}/unblock (spec 0116, data_contract). Invokable,
 * single action: no other verb exists on this route.
 */
class TaskUnblockController extends BaseApiController
{
    ...
    public function __invoke(UnblockTaskRequest $request, Task $task): JsonResponse
    {
        try {
            ...
            /** @var User $actor */
            $actor = $request->user();
            $task = $this->service->unblock($task, $actor);
