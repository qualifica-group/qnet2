...
use App\Http\Requests\Tasks\RejectTaskRequest;
...
/**
 * POST /api/tasks/{task}/reject (spec 0116, data_contract). Invokable,
 * single action: no other verb exists on this route.
 */
class TaskRejectController extends BaseApiController
{
    ...
    public function __invoke(RejectTaskRequest $request, Task $task): JsonResponse
    {
        try {
            ...
            /** @var User $actor */
            $actor = $request->user();
            $task = $this->service->reject($task, $actor);
