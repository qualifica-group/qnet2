...
use App\Http\Requests\Tasks\UncompleteTaskRequest;
...
/**
 * POST /api/tasks/{task}/uncomplete (spec 0116, data_contract). Invokable,
 * single action: no other verb exists on this route.
 */
class TaskUncompleteController extends BaseApiController
{
    ...
    public function __invoke(UncompleteTaskRequest $request, Task $task): JsonResponse
    {
        try {
            ...
            /** @var User $actor */
            $actor = $request->user();
            $task = $this->service->uncomplete($task, $actor);
