...
/**
 * Validates the payload for POST /api/task-priorities (spec 0101, D-4).
 *
 * Authorization is intentionally NOT handled here (it stays in the
 * controller via authorize('create', TaskPriority::class)).
...
class StoreTaskPriorityRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['required', 'string', 'max:'.self::NAME_MAX, Rule::unique('task_priorities', 'name')],
     ...
     */
    public function toData(): CreateTaskPriorityData
    {
        /** @var array<string, mixed> $validated */
        $validated = $this->validated();

        return CreateTaskPriorityData::fromValidated($validated);
    }
}
