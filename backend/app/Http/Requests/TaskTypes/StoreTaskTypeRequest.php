...
/**
 * Validates the payload for POST /api/task-types (spec 0101, D-4).
 *
 * Authorization is intentionally NOT handled here (it stays in the
 * controller via authorize('create', TaskType::class)).
...
class StoreTaskTypeRequest extends FormRequest
{
    ...
    {
        return [
            'name' => ['required', 'string', 'max:'.self::NAME_MAX, Rule::unique('task_types', 'name')],
     ...
     */
    public function toData(): CreateTaskTypeData
    {
        /** @var array<string, mixed> $validated */
        $validated = $this->validated();

        return CreateTaskTypeData::fromValidated($validated);
    }
}
