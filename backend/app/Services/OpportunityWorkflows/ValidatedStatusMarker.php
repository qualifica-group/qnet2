...
/**
 * The OPTIONAL 'validated' system row of an OpportunityWorkflowStatus set
 * (user directive 2026-08-03): unlike 'open'/'closed_won'/'closed_lost' — the
...
final class ValidatedStatusMarker
{
    ...
    public function apply(?int $workflowId, array $statusRows, Collection $existing): void
    {
        ...
        $current = $existing->first(
            static fn (OpportunityWorkflowStatus $status): bool => $status->system_key === WorkflowStatusSystemKey::Validated->value,
        );
     ...
     * @param  array<int, array{id: ?int, name: string, description: ?string, color: ?string, group: string, requires_note: bool, system_key: ?string, system_key_submitted?: bool}>  $statusRows
     */
    private function release(OpportunityWorkflowStatus $current, array $statusRows, bool $claimedByAnother): void
    {
     ...
     */
    private function create(?int $workflowId, array $marked): OpportunityWorkflowStatus
    {
        return OpportunityWorkflowStatus::query()->forceCreate([
