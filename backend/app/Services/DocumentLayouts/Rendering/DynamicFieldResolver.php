...
/**
 * Resolves the `custom_fields` and `opportunity_attributes` variable
 * categories (spec 0069's frozen catalogue, AC-042/043): the two DYNAMIC
 * categories, one variable per live `CustomFieldDefinition`/`Attribute`
 * rather than a fixed list — split out of VariableResolver to keep it under
 * the file-size soft limit (engineering.md §6).
 *
 * Both read from an already-resolved array accessor (`Quote::custom_fields`,
 * `Opportunity::attribute_values`), so a KEY absent from that array (spec
 * 0070 AC-237: a custom field whose definition was deleted after the layout
...
final class DynamicFieldResolver
{
    ...
    public function opportunityAttribute(string $key, Quote $quote): string
    {
        $attributeValues = $quote->opportunity?->attribute_values ?? [];
