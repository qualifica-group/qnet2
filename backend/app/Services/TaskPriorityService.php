...
use App\DataObjects\TaskPriorities\CreateTaskPriorityData;
...
/**
 * Business logic for the `task-priorities` resource (spec 0101, D-4): one of the four
...
class TaskPriorityService
{
    ...
    public function create(CreateTaskPriorityData $data): TaskPriority
    {
        $this->defaultRow->assertSelectable($data->isDefault, $data->isActive);

        return DB::transaction(function () use ($data): TaskPriority {
            /** @var TaskPriority $taskPriority */
            $taskPriority = TaskPriority::create([
                ...$data->attributes(),
                'sort_order' => $this->orderManager->placeNew(TaskPriority::class),
            ]);

            if ($data->isDefault) {
                $this->defaultRow->clearOthers(TaskPriority::class, $taskPriority->id);
            }
    ...
    public function update(TaskPriority $taskPriority, UpdateTaskPriorityData $data): TaskPriority
    {
        $isDefault = $data->isDefaultSubmitted ? $data->isDefault : $taskPriority->is_default;
        $isActive = $data->isActiveSubmitted ? $data->isActive : $taskPriority->is_active;
        $this->defaultRow->assertSelectable($isDefault, $isActive);

        return DB::transaction(function () use ($taskPriority, $data): TaskPriority {
            ...
            // no UPDATE query.
            $taskPriority->fill($attributes)->save();
    ...
    /**
     * Restrictive delete (spec 0101, D-8b): a task priority still referenced by a
     ...
     * through the SAME method via TaskPrioritiesTableDefinition::deleteModel().
     */
    public function delete(TaskPriority $taskPriority): void
    {
        if ($taskPriority->tasks()->exists()) {
            abort(409, 'This task priority is used by a task and cannot be deleted.');
        }
     ...
     * @return EloquentCollection<int, TaskPriority>
     */
    public function reorder(array $orderedIds): EloquentCollection
    {
        /** @var EloquentCollection<int, TaskPriority> $reordered */
        $reordered = $this->orderManager->reorder(TaskPriority::class, $orderedIds);

        return $reordered;
    }

    /**
     * Minimal, searchable, paginated task priority list for the for-select
    ...
    {
        $base = TaskPriority::query()->select(self::FOR_SELECT_COLUMNS);
        ...
        /** @var Collection<int, TaskPriority> $page */
     ...
     * `is_active` filter (a Task keeps showing its current task priority even
     * after it is deactivated), same projection applies. Total is unaffected.
     *
     * @param  Collection<int, TaskPriority>  $page
    ...
    private function appendHydratedIds(Collection $page, ForSelectQuery $query): Collection
    {
        ...
        /** @var Collection<int, TaskPriority> $hydrated */
        $hydrated = TaskPriority::query()
