 ...
 * 0072), shared verbatim by every status configurator (pipeline_statuses,
 * reward_statuses, contract_statuses): every mandatory row cannot be
...
class SystemStatusGuard
{
     ...
     */
    public function assertDeletable(PipelineStatus|RewardStatus|ContractStatus $status): void
    {
        if (! $status->isSystem()) {
            return;
        }
     ...
     * @throws HttpException 422
     */
    public function assertUpdatable(PipelineStatus|RewardStatus|ContractStatus $status, array $submittedAttributes): void
    {
        if (! $status->isSystem()) {
            return;
        }
