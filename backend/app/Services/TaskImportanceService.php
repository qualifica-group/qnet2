...
class TaskImportanceService
{
    ...
    /**
     * Restrictive delete (spec 0101, D-8b): a task importance still referenced by a
     ...
     * through the SAME method via TaskImportancesTableDefinition::deleteModel().
     */
    public function delete(TaskImportance $taskImportance): void
    {
        if ($taskImportance->tasks()->exists()) {
            abort(409, 'This task importance is used by a task and cannot be deleted.');
        }

        $taskImportance->delete();
    }
     ...
     * @return EloquentCollection<int, TaskImportance>
     */
    public function reorder(array $orderedIds): EloquentCollection
    {
        /** @var EloquentCollection<int, TaskImportance> $reordered */
        $reordered = $this->orderManager->reorder(TaskImportance::class, $orderedIds);

        return $reordered;
    }

    /**
     * Minimal, searchable, paginated task importance list for the for-select
    ...
    {
        $base = TaskImportance::query()->select(self::FOR_SELECT_COLUMNS);
        ...
        /** @var Collection<int, TaskImportance> $page */
     ...
     * `is_active` filter (a Task keeps showing its current task importance even
     * after it is deactivated), same projection applies. Total is unaffected.
     *
     * @param  Collection<int, TaskImportance>  $page
     * @return Collection<int, TaskImportance>
     */
    private function appendHydratedIds(Collection $page, ForSelectQuery $query): Collection
    {
        ...
        /** @var Collection<int, TaskImportance> $hydrated */
        $hydrated = TaskImportance::query()
