...
/**
 * The system rows an OpportunityWorkflowStatus set (a workflow's own, or the
 * global default set) carries (spec 0047, AC-004): an initial 'open' row and
 * the two terminal closed-outcome rows 'closed_won'/'closed_lost' — those
 * three are MANDATORY, pinned and non-deletable — plus the OPTIONAL
 * 'validated' row that closes the working phase before the outcome.
 *
 ...
 * client explicitly marks it (App\Services\OpportunityWorkflows\
 ...
 *
 * Persisted as `opportunity_workflow_statuses.system_key` (nullable — custom
 * rows have none). Never mass-assignable (only the service that creates/syncs
 * a workflow's status set writes it).
 */
