...
import { timeEntries as en } from '@/i18n/locales/en-time-entries'
import { timeEntries as itLocale } from '@/i18n/locales/it-time-entries'
...
/**
 * Spec 0122, AC-041: automates what would otherwise be a manual inspection,
 * same approach as `tasks-i18n-parity.test.ts`. `en: TranslationResources`
 * (via `en.ts`) only enforces STRUCTURAL parity (same keys) at compile time —
 * a translated-but-empty string would still typecheck.
 */
...
describe('time entries i18n parity (spec 0122 AC-041)', () => {
  ...
  it('exposes the navigation label timeEntries in en and it', () => {
    expect(enNavigation.timeEntries).toBeTruthy()
    expect(itNavigation.timeEntries).toBeTruthy()
  })
})
