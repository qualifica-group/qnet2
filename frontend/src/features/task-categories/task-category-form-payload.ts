...
import type { TaskCategoryFormValues } from '@/features/task-categories/use-task-category-form'
...
export function buildCreatePayload(values: TaskCategoryFormValues): CreateTaskCategoryPayload {
  return {
    name: values.name,
    color: values.color,
 ...
 * changed from the original task category. `sort_order` is NEVER
 * included: the backend 422s on its mere presence.
 */
export function buildUpdatePayload(
  values: TaskCategoryFormValues,
  original: TaskCategoryDetail,
): UpdateTaskCategoryPayload {
