 ...
 * `<TableView domain="task-categories">`, and the delete flow.
 *
...
const deleteTaskCategoryMock = vi.fn()
vi.mock('@/features/task-categories/api', () => ({
  deleteTaskCategory: (...args: unknown[]) => deleteTaskCategoryMock(...args),
  fetchTaskCategory: vi.fn(),
}))
...
describe('TaskCategoriesTable — reorder toggle (spec 0101 D-4, rectified)', () => {
  it('shows the reorder button with task-categories.update', () => {
    canMock.mockImplementation((permission) => permission === 'task-categories.update')

    renderTable()

    expect(
      screen.getByRole('button', { name: i18n.t('taskCategories.reorder.openButton') }),
    ).toBeInTheDocument()
  })

  it('hides the reorder button without task-categories.update', () => {
    canMock.mockReturnValue(false)

    renderTable()

    expect(
      screen.queryByRole('button', { name: i18n.t('taskCategories.reorder.openButton') }),
    ).not.toBeInTheDocument()
  })
})

describe('TaskCategoriesTable — mount', () => {
  it('mounts <TableView domain="task-categories">', () => {
    renderTable()

    expect(screen.getByRole('region', { name: 'table-task-categories' })).toBeInTheDocument()
  })
})

describe('TaskCategoriesTable — delete', () => {
  it("surfaces the backend's own message on a 409 (row used by a task, D-8b)", async () => {
    // Deliberately NOT the wording of `taskCategories.form.deleteInUse`: that static
    ...
    const message = 'This task category is used by 3 tasks and cannot be deleted.'
    deleteTaskCategoryMock.mockRejectedValue(
    ...
    await waitFor(() => expect(deleteTaskCategoryMock).toHaveBeenCalledWith(1))
  ...
  it('shows a forbidden toast on a 403', async () => {
    ...
    await waitFor(() =>
      expect(toastErrorMock).toHaveBeenCalledWith(i18n.t('taskCategories.form.deleteForbidden')),
    )
  })

  it('shows the success toast on a successful delete', async () => {
    deleteTaskCategoryMock.mockResolvedValue(undefined)

    renderTable()
    screen.getByRole('button', { name: 'delete row' }).click()

    await waitFor(() =>
      expect(toastSuccessMock).toHaveBeenCalledWith(i18n.t('taskCategories.form.deleted')),
    )
  })
})
 ...
 */
describe('TaskCategoriesTable — inactive badge copy', () => {
  const KEY = 'taskCategories.reorder.inactiveBadge'
