...
/**
 * Zod schema for the task category create/edit form, built as a factory so
...
function baseFields(t: TFunction) {
  return {
    name: z
      .string()
      .min(1, t('taskCategories.form.nameRequired'))
      .max(NAME_MAX_LENGTH, t('taskCategories.form.nameMax')),
    description: z
      .string()
      .max(DESCRIPTION_MAX_LENGTH, t('taskCategories.form.descriptionMax'))
      .nullable(),
    color: z
      .string()
      .min(1, t('taskCategories.form.colorRequired'))
      .refine((token) => swatchClassFor(token) !== undefined, t('taskCategories.form.colorInvalid')),
    icon: z
      .string()
...
/** Create schema. */
export function buildCreateTaskCategorySchema(t: TFunction) {
  return z.object(baseFields(t))
}

/** Edit schema (same shape; the partial PATCH is computed by the caller). */
export function buildUpdateTaskCategorySchema(t: TFunction) {
  return z.object(baseFields(t))
}

export type CreateTaskCategoryFormValues = z.infer<ReturnType<typeof buildCreateTaskCategorySchema>>
export type UpdateTaskCategoryFormValues = z.infer<ReturnType<typeof buildUpdateTaskCategorySchema>>
