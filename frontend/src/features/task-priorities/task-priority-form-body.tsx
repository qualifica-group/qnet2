...
import { useTaskPriorityForm } from '@/features/task-priorities/use-task-priority-form'
import type { TaskPriorityDetail, TaskPriorityFormMode } from '@/features/task-priorities/types'

interface TaskPriorityFormBodyProps {
...
/**
 * The task priority create/edit form UI. Every field is wrapped in `MetaField`
 ...
 * All non-render logic lives in `useTaskPriorityForm`.
 */
export function TaskPriorityFormBody({ mode, onSuccess, onCancel }: TaskPriorityFormBodyProps) {
  const { t } = useTranslation()
  const { field: fieldPermission } = useResourcePermissions()
  const { form, serverError, onSubmit } = useTaskPriorityForm({ mode, onSuccess })
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 p-4" noValidate>
          {identityVisible && (
            <FormSection
              icon={SignalHigh}
              title={t('taskPriorities.form.sections.identity.title')}
              description={t('taskPriorities.form.sections.identity.description')}
            >
              <MetaField
                control={form.control}
                name="name"
                metaKey="name"
                label={t('taskPriorities.form.name')}
              >
              ...
              <MetaField
                ...
                name="description"
                metaKey="description"
                label={t('taskPriorities.form.description')}
              >
                {({ field, disabled, readOnly }) => (
                  <FormControl>
                    <Textarea
              ...
              <MetaField
                ...
                name="color"
                metaKey="color"
                label={t('taskPriorities.form.color')}
              >
                {({ field, disabled }) => (
                  <FormControl>
                    <ColorTokenPicker
              ...
              <MetaField
                ...
                name="icon"
                metaKey="icon"
                label={t('taskPriorities.form.icon')}
              >
                {({ field, disabled, readOnly }) => (
                  <IconPickerControl
                    value={field.value}
              ...
              <MetaField
                ...
                name="is_active"
                metaKey="is_active"
                label={t('taskPriorities.form.isActive')}
                layout="inline"
              >
                {({ field, disabled }) => (
                  <FormControl>
              ...
              <MetaField
                ...
                name="is_default"
                metaKey="is_default"
                label={t('taskPriorities.form.isDefault')}
                hint={t('taskPriorities.form.isDefaultHint')}
                layout="inline"
              >
                {({ field, disabled }) => (
                  <FormControl>
          ...
          <div className="mt-auto flex justify-end gap-2 pt-2">
            ...
            >
              {t('taskPriorities.form.cancel')}
            </Button>
            <Button type="submit" disabled={form.formState.isSubmitting}>
              {form.formState.isSubmitting ? t('taskPriorities.form.saving') : t('taskPriorities.form.save')}
            </Button>
          </div>
        </form>
      </Form>
