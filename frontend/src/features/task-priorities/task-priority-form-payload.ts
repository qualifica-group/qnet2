...
import type { TaskPriorityFormValues } from '@/features/task-priorities/use-task-priority-form'
...
export function buildCreatePayload(values: TaskPriorityFormValues): CreateTaskPriorityPayload {
  return {
    name: values.name,
    color: values.color,
 ...
 * changed from the original task priority. `sort_order` is NEVER
 * included: the backend 422s on its mere presence.
 */
export function buildUpdatePayload(
  values: TaskPriorityFormValues,
  original: TaskPriorityDetail,
): UpdateTaskPriorityPayload {
  const payload: UpdateTaskPriorityPayload = {}
