...
/**
 * Zod schema for the task priority create/edit form, built as a factory so
...
function baseFields(t: TFunction) {
  return {
    name: z
      .string()
      .min(1, t('taskPriorities.form.nameRequired'))
      .max(NAME_MAX_LENGTH, t('taskPriorities.form.nameMax')),
    description: z
      .string()
      .max(DESCRIPTION_MAX_LENGTH, t('taskPriorities.form.descriptionMax'))
      .nullable(),
    color: z
      .string()
      .min(1, t('taskPriorities.form.colorRequired'))
      .refine((token) => swatchClassFor(token) !== undefined, t('taskPriorities.form.colorInvalid')),
    icon: z
      .string()
      .refine((name) => name === '' || isKnownIconName(name), t('taskPriorities.form.iconInvalid')),
    is_active: z.boolean(),
  }
}

/** Create schema. */
export function buildCreateTaskPrioritySchema(t: TFunction) {
  return z.object(baseFields(t))
}

/** Edit schema (same shape; the partial PATCH is computed by the caller). */
export function buildUpdateTaskPrioritySchema(t: TFunction) {
  return z.object(baseFields(t))
}

export type CreateTaskPriorityFormValues = z.infer<ReturnType<typeof buildCreateTaskPrioritySchema>>
export type UpdateTaskPriorityFormValues = z.infer<ReturnType<typeof buildUpdateTaskPrioritySchema>>
