...
describe('computeProductMargins', () => {
  it('attributes a cost to its product row and computes the margin', () => {
    const productLines: ProductMarginProductInput[] = [{ key: 'a', productName: 'Widget', rowNumber: 1, net: 100 }]
    ...
    expect(rows).toEqual([{ key: 'a', productName: 'Widget', rowNumber: 1, revenueNet: 100, costNet: 30, margin: 70 }])
  ...
  it('buckets an unassociated cost as generic', () => {
    ...
    expect(rows[0]).toMatchObject({ costNet: 0, margin: 100 })
  ...
  it('renders a negative margin when the imputed costs exceed the revenue', () => {
    const productLines: ProductMarginProductInput[] = [{ key: 'a', productName: 'Widget', rowNumber: 1, net: 20 }]
  ...
  it('sums several costs attributed to the same product row', () => {
    ...
    expect(computeProductMargins(productLines, costLines).rows[0]).toMatchObject({ costNet: 25, margin: 75 })
  })
  ...
  it('invariant: sum(margins) - genericCostNet == revenue.net - cost.net, whatever the mix', () => {
    const productLines: ProductMarginProductInput[] = [
      { key: 'a', productName: 'Widget', rowNumber: 1, net: 100 },
      { key: 'b', productName: 'Gadget', rowNumber: 2, net: 50 },
    ]
    const costLines: ProductMarginCostInput[] = [
...
describe('productLinesFromFormOfferLines / costLinesFromFormCostLines', () => {
  ...
  it('resolves the product name through the caller-provided cache', () => {
    const rows = productLinesFromFormOfferLines(
      [formOfferRow({ product_id: 7, client_key: 'line-7' })],
      (id) => (id === 7 ? 'Widget Pro' : null),
    )
    expect(rows).toEqual([{ key: 'line-7', productName: 'Widget Pro', rowNumber: 1, net: 10 }])
  })
...
describe('QuoteProductMargins (component)', () => {
  ...
  it('renders one row per product plus the generic-costs row, negative margin highlighted', () => {
    render(
      <QuoteProductMargins
        rows={[
          { key: 'a', productName: 'Widget', rowNumber: 1, revenueNet: 100, costNet: 30, margin: 70 },
          { key: 'b', productName: 'Gadget', rowNumber: 2, revenueNet: 20, costNet: 35, margin: -15 },
        ]}
        genericCostNet={12}
      />,
    )
  ...
  it('falls back to a generic product label when the name could not be resolved', () => {
    render(
      <QuoteProductMargins
        rows={[{ key: 'a', productName: null, rowNumber: 1, revenueNet: 10, costNet: 0, margin: 10 }]}
        genericCostNet={0}
      />,
    )
