 ...
 * `opportunityWorkflows.criterionFields.` prefix; everything else — including
...
function renderCriteriaFields(value: unknown) {
  ...
  const Cell = opportunityWorkflowColumnRenderers.criteria_fields
...
describe('CriteriaFieldsCell', () => {
  it('translates a native entry and renders a custom entry literally', () => {
    const { getByLabelText } = renderCriteriaFields([
      'opportunityWorkflows.criterionFields.state_id',
      'Preferred supplier',
    ])
  ...
  it('does not translate a custom label that coincides with an existing, unrelated i18n key', () => {
    ...
    const { getByLabelText } = renderCriteriaFields(['opportunityWorkflows.detail.title'])
    expect(getByLabelText('opportunityWorkflows.detail.title')).toBeInTheDocument()
  })
