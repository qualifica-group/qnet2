...
export function buildCreatePayload(values: TaskImportanceFormValues): CreateTaskImportancePayload {
  return {
    name: values.name,
    color: values.color,
 ...
 * changed from the original task importance. `sort_order` is NEVER
 * included: the backend 422s on its mere presence.
 */
export function buildUpdatePayload(
  values: TaskImportanceFormValues,
  original: TaskImportanceDetail,
): UpdateTaskImportancePayload {
  const payload: UpdateTaskImportancePayload = {}
