...
describe('buildCreateTaskImportanceSchema (spec 0101)', () => {
  it('accepts a valid set of values', () => {
    const schema = buildCreateTaskImportanceSchema(i18n.t)
...
describe('buildUpdateTaskImportanceSchema', () => {
  it('has the same shape as the create schema', () => {
    const schema = buildUpdateTaskImportanceSchema(i18n.t)
    expect(schema.safeParse(VALID_VALUES).success).toBe(true)
  })
})
