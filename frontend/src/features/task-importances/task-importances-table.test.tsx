 ...
 * `<TableView domain="task-importances">`, and the delete flow.
 *
...
const deleteTaskImportanceMock = vi.fn()
vi.mock('@/features/task-importances/api', () => ({
  deleteTaskImportance: (...args: unknown[]) => deleteTaskImportanceMock(...args),
  fetchTaskImportance: vi.fn(),
}))
...
beforeEach(() => {
  ...
  deleteTaskImportanceMock.mockReset()
  toastSuccessMock.mockReset()
  toastErrorMock.mockReset()
})

describe('TaskImportancesTable — reorder toggle (spec 0101 D-4, rectified)', () => {
  it('shows the reorder button with task-importances.update', () => {
    canMock.mockImplementation((permission) => permission === 'task-importances.update')

    renderTable()

    expect(
      screen.getByRole('button', { name: i18n.t('taskImportances.reorder.openButton') }),
    ).toBeInTheDocument()
  })

  it('hides the reorder button without task-importances.update', () => {
    canMock.mockReturnValue(false)

    renderTable()

    expect(
      screen.queryByRole('button', { name: i18n.t('taskImportances.reorder.openButton') }),
    ).not.toBeInTheDocument()
  })
})

describe('TaskImportancesTable — mount', () => {
  it('mounts <TableView domain="task-importances">', () => {
    renderTable()

    expect(screen.getByRole('region', { name: 'table-task-importances' })).toBeInTheDocument()
  })
})

describe('TaskImportancesTable — delete', () => {
  it("surfaces the backend's own message on a 409 (row used by a task, D-8b)", async () => {
    // Deliberately NOT the wording of `taskImportances.form.deleteInUse`: that static
    ...
    const message = 'This task importance is used by 3 tasks and cannot be deleted.'
    deleteTaskImportanceMock.mockRejectedValue(
    ...
    await waitFor(() => expect(deleteTaskImportanceMock).toHaveBeenCalledWith(1))
  ...
  it('shows a forbidden toast on a 403', async () => {
    ...
    await waitFor(() =>
      expect(toastErrorMock).toHaveBeenCalledWith(i18n.t('taskImportances.form.deleteForbidden')),
    )
  })

  it('shows the success toast on a successful delete', async () => {
    deleteTaskImportanceMock.mockResolvedValue(undefined)

    renderTable()
    screen.getByRole('button', { name: 'delete row' }).click()

    await waitFor(() =>
      expect(toastSuccessMock).toHaveBeenCalledWith(i18n.t('taskImportances.form.deleted')),
    )
  })
})
 ...
 */
describe('TaskImportancesTable — inactive badge copy', () => {
  const KEY = 'taskImportances.reorder.inactiveBadge'
