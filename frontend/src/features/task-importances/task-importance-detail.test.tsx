...
describe('TaskImportanceDetailView — detail fields', () => {
  it('shows the name, description, color token, icon, order and both timestamps', () => {
    render(<TaskImportanceDetailView taskImportance={taskImportance()} />)
  ...
  it('shows the em-dash placeholder for an empty description and an unset icon', () => {
    render(<TaskImportanceDetailView taskImportance={taskImportance({ description: null, icon: null })} />)

    expect(screen.getAllByText('—')).toHaveLength(2)
  })
})

describe('TaskImportanceDetailView — activity log section', () => {
  it('mounts the section for the viewed row when view_activity is granted', () => {
    render(
      <TaskImportanceDetailView
        taskImportance={taskImportance({
          permissions: { ...taskImportance().permissions, actions: { view_activity: true } },
        })}
      />,
    )

    expect(screen.getByText('Activity log')).toBeInTheDocument()
    expect(activityLogSectionMock).toHaveBeenCalledWith({ resource: 'task-importances', id: 4 })
  })
