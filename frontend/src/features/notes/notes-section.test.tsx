...
import {
  ...
  pasteText,
  renderSection,
} from '@/features/notes/notes-section-test-helpers'

/**
 * Spec 0052 AC-070/AC-071 — `<NotesSection>` mounted end-to-end (real
 * `useNotes`/`useNote*Mutations` hooks and F2's `Mention` extension, only the
 * API module is mocked), mirroring `request-work-panel.test.tsx`'s pattern.
 * Quote-scope selector coverage lives in `notes-section-quote-scope.test.tsx`
 * (engineering.md §6: file kept under the size limit).
 */
...
describe('NotesSection — reply flow (AC-071, D-7)', () => {
  it('replying to a root sends the body and the ROOT id as parent_id', async () => {
    ...
    // The reply composer's field is the second body editor on the page (the
    // root composer's own field is always mounted first).
    const fields = noteBodyEditors()
    expect(fields).toHaveLength(2)
    pasteText(fields[1] as HTMLElement, 'A reply text')

    const sendButtons = screen.getAllByRole('button', { name: 'Send' })
    await waitFor(() => expect(sendButtons[1]).not.toBeDisabled())
    ...
    await waitFor(() =>
      expect(createNoteMock).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          entity_id: 7,
          body: expect.stringContaining('A reply text'),
          parent_id: 5,
        }),
      ),
    )
...
describe('NotesSection — onThreadChanged (host notes_count)', () => {
  it('fires after a new root note is created', async () => {
    ...
    pasteText(noteBodyEditor(), 'Fresh root')
  ...
  it('does not fire when a note is only edited: the count is unchanged', async () => {
    ...
    const fields = noteBodyEditors()
    pasteText(fields[1] as HTMLElement, ' Edited body')
