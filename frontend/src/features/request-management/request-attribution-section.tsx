 ...
 * 2026-07-22): "Fonte" and "Segnalatore". Every control is metadata-driven, so
 * its per-field gating comes from the server-derived `permissions` block like
 * every other field here.
 *
 * The team left this section with spec 0097 rev-2 D-7
 * (`request-team-section.tsx`) and the Sede followed it with the user
 * directive 2026-09-10 (`request-site-section.tsx`): it is what SCOPES the
 * operator slots, so it now sits directly above them in the side column. Their
 * link is cabled by whoever owns the form (`useRequestSiteOperatorLink`),
 * never by a section: kept inside one, the other would stop reacting to it.
 *
