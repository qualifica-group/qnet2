...
import { deleteTag } from '@/features/tags/api'
 ...
 * `tags` domain, its custom cell renderers and a row-action handler, and
 * delegates the open mode (modal Sheet vs dedicated page) of view/edit/
 * create to `useModuleOpener`, resolved from the user's preference (spec
 * 0042). It still owns the delete flow (confirm + toast + grid refresh,
 * mapping the backend's restrictive-delete 409/422 when the tag is still
 * attached to a record) and refreshes the SSRM grid after every mutation.
