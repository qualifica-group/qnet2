import { useTranslation } from 'react-i18next'
import { useWatch, type Control } from 'react-hook-form'
import { FolderTree, Hash, Loader2, TriangleAlert } from 'lucide-react'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { RECORD_HEADER_CLASS } from '@/components/record-form/layout'
import type { FlatCategoryOption } from '@/features/product-categories/flatten-tree'
import type { ProductFormValues } from '@/features/products/use-product-form'

interface ProductFormHeaderProps {
  control: Control<ProductFormValues>
  /** Drives the heading pair (create/edit); the badges are live in both modes. */
  isEdit: boolean
  /** The picker's own option list: the only place a category id has a NAME here. */
  categoryOptions: FlatCategoryOption[]
  /** id of the RHF `<form>` the save action attaches to via the HTML `form=` attribute. */
  formId: string
  isSubmitting: boolean
  submitError: string | null
  onCancel: () => void
}

/**
 * Identity bar of the product form — the same bar the Opportunita' and
 * Gestione Richieste screens carry (`RECORD_HEADER_CLASS`): heading and live
 * identity pills on the left, save/cancel on the right, a refused submit
 * reported right under the button that was pressed.
 *
 * This bar is the form's ONE heading: the module is registered
 * `formOwnsHeader`, so the dedicated page drops its own title/subtitle block
 * and the Sheet keeps its `SheetHeader` `sr-only`. It reuses the hosts' own
 * `form.createTitle`/`editTitle` strings.
 *
 * The pills read LIVE from the form (code and category), never the persisted
 * snapshot: while the operator retypes the code the bar must not keep naming
 * the old one.
 */
export function ProductFormHeader({
  control,
  isEdit,
  categoryOptions,
  formId,
  isSubmitting,
  submitError,
  onCancel,
}: ProductFormHeaderProps) {
  const { t } = useTranslation()
  const code = useWatch({ control, name: 'code' })
  const categoryId = useWatch({ control, name: 'category_id' })

  const categoryName = categoryOptions.find((option) => option.id === categoryId)?.name ?? null

  return (
    <header className={RECORD_HEADER_CLASS}>
      <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-1">
        <div className="flex min-w-0 flex-col">
          <h1 className="min-w-0 truncate text-base font-semibold">
            {t(isEdit ? 'products.form.editTitle' : 'products.form.createTitle')}
          </h1>
          <p className="min-w-0 truncate text-sm text-muted-foreground">
            {t(isEdit ? 'products.form.editSubtitle' : 'products.form.createSubtitle')}
          </p>
        </div>

        {code ? (
          <Badge variant="outline" className="h-5 min-h-5 max-w-full gap-1.5">
            <Hash className="size-3" aria-hidden="true" />
            <span className="truncate">{code}</span>
          </Badge>
        ) : null}
        {categoryName ? (
          <Badge variant="outline" className="h-5 min-h-5 max-w-full gap-1.5">
            <FolderTree className="size-3" aria-hidden="true" />
            <span className="truncate">{categoryName}</span>
          </Badge>
        ) : null}
      </div>

      <div className="ml-auto flex shrink-0 items-center gap-2">
        <Button type="button" variant="secondary" onClick={onCancel} disabled={isSubmitting}>
          {t('products.form.cancel')}
        </Button>
        <Button type="submit" form={formId} disabled={isSubmitting}>
          {isSubmitting && <Loader2 className="size-4 animate-spin" aria-hidden="true" />}
          {isSubmitting ? t('products.form.saving') : t('products.form.save')}
        </Button>
      </div>

      {submitError && (
        <div
          role="alert"
          className="flex w-full items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm font-medium text-destructive"
        >
          <TriangleAlert className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
          {submitError}
        </div>
      )}
    </header>
  )
}
